...
class Trainer(object):
    ...
    def _train(self, epoch_idx):
        ...
        for batch_idx, batch in enumerate(zip(*self.train_loader)):
            ...
            for param in self.model.parameters():
                ...
                if torch.isnan(param.grad).float().sum() != 0:
                    param.grad.zero_()
    ...
    def _init_files(self, config):
        ...
        if self.config["resume"]:
            ...
            if config["cross"] == True:
                mlp_pre = str(int(config["mlp_pre"]))
            else: 
                mlp_pre = ''
            ##
            ...
            base_dir = "{}-{}-{}-{}-{}-{}".format(
                ...
                config['mlp_layer']['name'] + '_' + str(config['mlp_layer']['use']) + mlp_pre,
                config["way_num"],
                config["shot_num"],
            )
            ##
            ...
            domain_path = os.path.join(config["result_root"], config["domain_set"])
            if not os.path.exists(domain_path):
                os.makedirs(domain_path)

            model_path = os.path.join(
                domain_path, config["classifier"]["name"]
            )
    ...
    def _init_model(self, config):
        ...
        # bsh:
        if config['mlp_layer']['kwargs'] != None and config['mlp_layer']['use'] == True:
    ...
    def _init_optim(self, config):
        ...
        if config["cross"] == False:
            ...
            params_dict_list.append(
                {
                    "params": filter(
                        lambda p: id(p) not in params_idx, self.model.classifier.parameters()
                    )
                }
            )
        ...
        if self.config["resume"]:
            ...
            scheduler.load_state_dict(all_state_dict["lr_scheduler"])
