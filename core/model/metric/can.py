...
def one_hot(indices, depth, use_cuda=True, device=None):
    if use_cuda:
        encoded_indicies = torch.zeros(indices.size() + torch.Size([depth])).cuda(device)
    else:
...
class CAN(MetricModel):
    ...
    def set_forward(
        ...
        support_targets_one_hot = one_hot(
            ...
            self.way_num, device=self.device
        )
    ...
    def set_forward_loss(self, batch):
        ...
        emb = self.emb_func(images)  # [320, 640, 5, 5]
        b, c, h, w = emb.size()

        if hasattr(self, 'mlp_layer') and self.cross == False:
            emb = self.mlp_layer(emb.reshape(-1, 640)).reshape(b, c, h, w)
        ...
        loss1 = self.loss_func(output, query_global_targets.contiguous().reshape(-1), self.device)
        loss2 = self.loss_func(cls_scores, query_targets.reshape(-1), self.device)
