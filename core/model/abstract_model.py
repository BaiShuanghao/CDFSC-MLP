...
class AbstractModel(nn.Module):
    ...
    @abstractmethod
    def set_forward_vis(self, *args, **kwargs):
        pass
    
    @abstractmethod
    def ata_forward(self, batch):
        pass
    
    def forward(self, x, vis=False):
        if self.training and vis == False:
            return self.set_forward_loss(x)
        elif vis == False:
            return self.set_forward(x)
        else:
            return self.set_forward_vis(x)
    ...
    def split_by_episode(self, features, mode):
        ...
        if mode == 1:  # input 2D, return 3D(with episode) E.g.ANIL & R2D2
            ...
            features = features.view(self.way_num, self.shot_num + self.query_num, -1)  # (C, F)
            support_features = (
                features[:, : self.shot_num, :]
                .contiguous()
