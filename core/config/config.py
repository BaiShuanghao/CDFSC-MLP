...
class Config(object):
    ...
    def __init__(self, config_file=None, variable_dict=None, is_resume=False, is_train=True, is_visual=False):
        ...
        if config_file == None:
            if is_train == True and is_resume == False:   # Train
                config_file = self.console_dict['config_dir'] + '/' + \
                "mini" + '/' + self.console_dict['config_name']
            else:   # Test and Test_visual
                self.config_dir = self.console_dict['res_dir'] + '/' + \
                    self.console_dict['domain_set'] + '/' + self.console_dict['res_config_dir'].split('-')[0] + '/' + \
                    self.console_dict['res_config_dir']
    ...
    def get_config_dict(self):
        ...
        if self.is_train == True:
            ...
            return self.config_dict, self.config_dir
    ...
    def _load_console_dict():
        ...
        parser.add_argument("-aug", "--augment", type=bool,
                            help="use augment or not")
        parser.add_argument(
            "-aug_times",
            "--augment_times",
            type=int,
        ...
        # bsh
        parser.add_argument("-cross", action='store_true', help="cross domain setting")
        parser.add_argument("-mlp_pre", action='store_true', help="")
        parser.add_argument("-config_dir", type=str,
                            default='./our_config', help="dir of config")
        parser.add_argument("-domain_set", type=str,
                            default='mini', help="setting of domain")
        parser.add_argument("-config_name", type=str, help="name of config")
        parser.add_argument("-res_dir", type=str, default='./results', help="dir of result config")
        parser.add_argument("-res_config_dir", type=str, help="name of result config")
        parser.add_argument("-dataset", type=str, default='miniImgaNet', help="choose dataset to test")

        # TODO: n_gpu should be len(gpus)?
        parser.add_argument("-n_gpu", type=int, help="gpu num")
        parser.add_argument("-seed", type=int, help="seed")
        parser.add_argument("-deterministic", type=bool,
                            help="deterministic or not")
    ...
    def _merge_config_dict(self):
        ...
        if self.is_resume:
            config_dict["resume_path"] = self.config_file[: -
                                                          1 * len("/config.yaml")]
